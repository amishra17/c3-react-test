...
describe("UserProfile", () => {
            ...
            it('should call on delete callback on clicking delete button', () => {
                    ...
                    let __component = shallow( < UserProfile onDeleteClick = {onDelete}/>);
                     __component.find('a').simulate('click', event); 
            ...
            it('check deafult label', () => {
                ...
                expect(__component.find('label').prop("innerText")).toEqual(__defaultProps.names)
            })

            it('check deafult avatar', () => {
                let __component = shallow( < UserProfile / > )
                expect(__component.find('img').prop("src")).toEqual(__defaultProps.avatar)
            })
            ...
            it('should render correctly when supplied props', () => {
                    ...
                    let __component = shallow( < UserProfile { ...props
                        ...
                        />)
                        expect(__component.find('label').prop("innerText")).toEqual(props.names); 
                        expect(__component.find('img').prop("src")).toEqual(props.avatar)
            })

            it('Should render avatar', () => {
                let __component = shallow( < UserProfile / > )
                expect(__component.find('img')).toHaveLength(1)
            })
