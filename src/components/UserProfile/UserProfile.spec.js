...
describe("UserProfile", () => {
    ...
    it('should call on delete callback on clicking delete button', () => {
        ...
        let __component = shallow( < UserProfile onDeleteClick = { onDelete } />);
            __component.find('.delete-btn a').simulate('click', event); expect(onDelete).toHaveBeenCalledWith(event); expect(onDelete).toHaveBeenCalledTimes(1);
    })
    ...
    it('should call on delete callback only once on clicking delete button', () => {
        ...
        let __component = shallow( < UserProfile onDeleteClick = { onDelete } />);
         __component.find('.delete-btn a').simulate('click', event); expect(onDelete).toHaveBeenCalledTimes(1);
    })
